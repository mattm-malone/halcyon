...
export const ThemePreview: React.FC<ThemePreviewProps> = ({ themeName, themeType, isSelected, onClick }) => {
  ...
  useEffect(() => {
    ...
    // Load colors for this specific theme
    const loadThemeColors = () => {
      ...
      colorSettings.forEach(setting => {
        const color = themeManager.getThemeColor(themeName, setting, themeType);
        if (color) {
          newColors[setting] = color.startsWith('#') ? color : `#${color}`;
        }
      });
    ...
    loadThemeColors();
  }, [themeManager, isLoading, themeType, themeName]);
      ...
      <label htmlFor={`${themeType}-${themeName}`} className="preset-label">
        <div className="svg-preview-container">
          ...
          >
            {/* Background */}
            <path
              fill={colors.SETTING_BG_COLOR || '#FFFFFF'}
            ...
            {themeType === 'day' ? (
              <path
                fill={colors.SETTING_RING_DAY_COLOR || '#00AAFF'}
                d="M0 0h144v82H0z"
              />
            ) : (
              <path
                fill={colors.SETTING_RING_NIGHT_COLOR || '#0055AA'}
                d="M0 86h144v82H0z"
              />
            )}
            ...
            <path
              fill={colors.SETTING_RING_SUNRISE_COLOR || '#FFAAAA'}
              d="M19 72v25H0V72h19Z"
            />
            <path
              fill={colors.SETTING_RING_SUNSET_COLOR || '#FFAA01'}
              d="M144 72v25h-19V72h19Z"
            />

            {/* Sun */}
            <path
              fill={colors.SETTING_SUN_FILL_COLOR || '#FFFF00'}
              d="M52 8.5a8.5 8.5 0 1 1-17 0 8.5 8.5 0 0 1 17 0Z"
            />
            <path
              fill={colors.SETTING_SUN_STROKE_COLOR || '#000000'}
              fillRule="evenodd"
            ...
            {/* Pips */}
            <path
              fill={colors.SETTING_PIP_COLOR_SECONDARY || '#AAAAAA'}
            ...
            />
            <path
              fill={colors.SETTING_PIP_COLOR_PRIMARY || '#000000'}
              d="M17 83h6v3h-6v-3ZM70 23v-5h3v5h-3ZM70 151v-6h3v6h-3ZM121 83h6v3h-6v-3Z"
            />

            {/* Ring Stroke */}
            <g fill={colors.SETTING_RING_STROKE_COLOR || '#000000'}>
