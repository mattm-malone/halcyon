...
export const SVGPreview: React.FC<SVGPreviewProps> = ({ themeType, title }) => {
  const { isLoading, getThemeColors } = useTheme();
  const colors = getThemeColors(themeType);

  // Memoize color settings that this SVG needs
  const colorSettings = useMemo(() => [
    ...
    themeType === 'day' ? 'SETTING_RING_DAY_COLOR' : 'SETTING_RING_NIGHT_COLOR',
  ...
  // Get colors from theme colors, with fallbacks
  const getColor = (setting: string, fallback: string) => {
    return colors[setting] || fallback;
  };

  if (isLoading) {
    return <div className="svg-preview-loading">Loading preview...</div>;
  }
    ...
    <div className="svg-preview-container">
      ...
      <div className="svg-wrapper">
        <svg 
          ...
          viewBox="0 0 144 168"
          fill="none"
          className="watchface-preview"
        >
          {/* Background */}
          <path 
            id="BG_COLOR" 
            fill={getColor('SETTING_BG_COLOR', '#FFFFFF')} 
          ...
          {themeType === 'day' ? (
            <path 
              id="RING_DAY_COLOR" 
              fill={getColor('SETTING_RING_DAY_COLOR', '#00AAFF')} 
              d="M0 0h144v82H0z"
            />
          ) : (
            <path 
              id="RING_NIGHT_COLOR" 
              fill={getColor('SETTING_RING_NIGHT_COLOR', '#0055AA')} 
              d="M0 86h144v82H0z"
            />
          )}
          ...
          <path 
            id="RING_SUNRISE_COLOR" 
            fill={getColor('SETTING_RING_SUNRISE_COLOR', '#FFAAAA')} 
            d="M19 72v25H0V72h19Z"
          />
          <path 
            id="RING_SUNSET_COLOR" 
            fill={getColor('SETTING_RING_SUNSET_COLOR', '#FFAA01')} 
          ...
          <path 
            id="SUN_FILL_COLOR" 
            fill={getColor('SETTING_SUN_FILL_COLOR', '#FFFF00')} 
            d="M52 8.5a8.5 8.5 0 1 1-17 0 8.5 8.5 0 0 1 17 0Z"
          />
          <path 
            id="SUN_STROKE_COLOR" 
            fill={getColor('SETTING_SUN_STROKE_COLOR', '#000000')} 
            fillRule="evenodd" 
          ...
          {/* Pips */}
          <path 
            id="PIP_COLOR_SECONDARY" 
            fill={getColor('SETTING_PIP_COLOR_SECONDARY', '#AAAAAA')} 
          ...
          />
          <path 
            id="PIP_COLOR_PRIMARY" 
            fill={getColor('SETTING_PIP_COLOR_PRIMARY', '#000000')} 
            d="M17 83h6v3h-6v-3ZM70 23v-5h3v5h-3ZM70 151v-6h3v6h-3ZM121 83h6v3h-6v-3Z"
          />
          
          {/* Ring Stroke */}
          <g id="RING_STROKE_COLOR" fill={getColor('SETTING_RING_STROKE_COLOR', '#000000')}>
